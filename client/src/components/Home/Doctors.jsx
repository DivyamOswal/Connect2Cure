...
const Doctors = () => {
  ...
  return (
    <div className="bg-[#E6E6E6] px-6 py-5 md:px-16 lg:px-24 xl:px-32 overflow-hidden">
      <Title
        title={t("title", { defaultValue: "Meet Our Doctors" })}
        subtitle={t("subtitle", {
          defaultValue: "Your Health, Our Expert Hands",
        })}
      />
      ...
      <div className="mx-auto">
        {loading && (
          <p className="text-center text-sm text-gray-600 py-4">
            {t("loading", { defaultValue: "Loading doctors..." })}
          </p>
        )}
        ...
        {!loading && doctors.length === 0 && (
          <p className="text-center text-sm text-gray-600 py-4">
            {t("empty", {
              defaultValue: "No doctors available right now.",
            })}
          </p>
        )}
                      ...
                      <div className="p-3">
                        ...
                        {doctor.fee !== undefined && (
                          <p className="text-sm font-semibold text-gray-900 mt-2">
                            {t("fee", { defaultValue: "Fee" })}: ₹{doctor.fee}
                          </p>
                        )}
                      </div>
                    </div>
        ...
        <div className="flex items-center justify-center pt-7">
          <Link to="/doctors">
            <button className="border px-6 py-2 rounded hover:border-[#FF8040] hover:bg-white transition-colors">
              {t("moreDoctors", { defaultValue: "More Doctors" })}
            </button>
          </Link>
        </div>
      </div>
