...
const HomeFeatures = () => {
  ...
  useEffect(() => {
    const updateSize = () => {
      setItemsToShow(window.innerWidth >= 1024 ? 4 : 2);
    };
    updateSize();
  ...
  return (
    <div className="bg-[#E6E6E6] px-6 py-5 md:px-16 lg:px-24 xl:px-32 overflow-hidden">
      <Title
        title={t("title", {
          defaultValue: "Everything You Need in One Healthcare Platform",
        })}
        subtitle={t("subtitle", {
          defaultValue:
            "Manage patients, appointments, billing, and records seamlessly with our secure and intuitive system.",
        })}
      />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 py-4">
        {featuresData.slice(0, itemsToShow).map((feature) => (
          ...
          >
            <div className="text-2xl mb-4 hidden md:block">
              {feature.icon}
            </div>
